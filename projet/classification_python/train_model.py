...
def opt_parse():
    ...
    parser.add_argument("--estimator", nargs='+', choices=['random_forest', 'logistic_regression', 'svm', 'neural_network', 'decision_tree_classifier', 'all'], required=True, help="Estimator type to train")
...
def train_model(estimator, X_train, y_train, param_grid=None):
    ...
    if model.__class__.__name__ in ["RandomForestClassifier", "DecisionTreeClassifier"]:
        ...
        c_source = model_c.save(file='model_c/'+model.__class__.__name__+'.h')
...
def save_dataset_train_and_test(X_train, y_train, X_test, y_test):
    f = open("csv_files/dataset_train.csv", "w")
    ...
    f = open("csv_files/dataset_test.csv", "w")
...
def main():
    ...
    for e in estimator:
        ...
        save_model(model, "model_python/"+str(model.__class__.__name__)+'.joblib')
